...
public class Game {
	...
	public static void flush() {
		imageMap = new HashMap<>(); // Map of preloaded images
		
		bulletMap = new HashMap<>(); // Projectile types
		activeEnemyBullets = new ArrayList<>(); // Projectiles currently alive
		deadEnemyBullets = new ArrayList<>(); // Projectiles to be killed
		
		activePlayerBullets = new ArrayList<>(); // Projectiles currently alive
		deadPlayerBullets = new ArrayList<>(); // Projectiles to be killed
		
		routineMap = new HashMap<>(); // Routine types
		
		enemyMap = new HashMap<>(); // Enemy types
		activeEnemies = new ArrayList<>(); // Enemies currently alive
		deadEnemies = new ArrayList<>(); // Enemies to be killed
		
		scriptMap = new HashMap<>(); // Script types
		scriptQueue = new ArrayDeque<>(); // Queue of scripts, scripts are queues of subscripts
		activeScript = new ArrayDeque<>(); // Current script
		
		activePickups = new ArrayList<>(); // Pickups currently alive
		deadPickups = new ArrayList<>(); // Pickups currently alive
		
		clipMap = new HashMap<>(); // Map of sound clip delays
		currentSong = null;
		stageSong = "";
		
		gameSpeed = 1.0; // FPS multiplier
		frameCount = 0; // Gets reset after each script
		globalFrameCount = 0; // Is always incrementing
		lastSpawnFrame = 0;
		run = true;
	}
	
	public static void end(boolean adsfg) {
		// Stops the game
	...
	public static void runScript() {
		...
		if (activeScript.peekFirst() == null)
			if (scriptQueue.peekFirst() != null && activeEnemies.isEmpty()) // Goes to next script
				...
				System.out.println("lkajwhserf fjkhsadlhkjl");
				if (fuckyou) {
					System.out.println("asdfg sdagfhsdgf");
					Game.end(false);
				} else
					Game.end(true);
				return;
