...
public class Game extends JPanel implements Runnable, KeyListener {
    ...
    public Game(String trackFile) {
        ...
        // Personal best times
        personalBest = RMain.getPersonalBest(track.getTrackID());
        ...
        exitLabel.setFont(RMain.uiTextMedium);
        exitLabel.setForeground(Color.WHITE);
        exitLabel.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseClicked(MouseEvent e) {
                stopMusic();
                RMain.saveDataToFile();
                RMain.exitFromGame();
            }
        });
        ...
        restartLabel.setFont(RMain.uiTextMedium);
        ...
        if (player1.isFinished && !haveParsedFinish) {
            ...
            if (personalBest != null) {
                ...
                if (timeObj.compareTo(personalBest) < 0) {
                    ...
                    RMain.addPersonalData(track.getTrackID(), latestTime);
                    personalBest = latestTime;
                }
            } else {
                // If PB does not exist, add it
                RMain.addPersonalData(track.getTrackID(), timeObj);
                personalBest = latestTime;
            }
    ...
    public void paintComponent(Graphics g) {
        ...
        if (gameLoaded) {
            ...
            if (startingState) {
                ...
                drawCenteredText(g2D, "Press any key to start...", 72, RMain.uiTextBig);
            }
            
            if (!player1.isFinished) {
                // Anything in here only displays if the player is still going
                if (player1.isBoosting) {
                    drawCenteredText(g2D, "BOOST", 72, RMain.uiTextBig);
                } else if (player1.isNoControl && !startingState) {
                    drawCenteredText(g2D, "NO CONTROL", 72, RMain.uiTextBig);
                }
                ...
                g2D.setFont(RMain.uiTextMediumHighlight);
                ...
                g2D.setColor(Color.WHITE);
                g2D.setFont(RMain.uiTextMediumHighlight);
                drawCenteredText(g2D, "Time: " + timeObj, 290, RMain.uiTextMediumHighlight);
                if (personalBest == null) {
                    drawCenteredText(g2D, "Personal Best: " + new Time(0, 0, 0), 365, RMain.uiTextMediumHighlight);
                } else {
                    drawCenteredText(g2D, "Personal Best: " + personalBest, 365, RMain.uiTextMediumHighlight);
                }
